...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        let platform_items = [
            ("Oil & Filter Change", 10000, 12, "[\"undercar\"]", "Factory interval"),
            ("Spark Plugs", 60000, 0, "[\"engine_bay\"]", ""),
            ("Air Filter", 20000, 24, "[\"engine_bay\"]", ""),
            ("Cabin Air Filter", 20000, 24, "[\"interior\"]", "Behind glove box"),
            ("Brake Fluid Flush", 0, 24, "[\"brakes\"]", "VW recommends every 2 years"),
            ("Coolant", 100000, 0, "[\"engine_bay\"]", ""),
            ("Serpentine Belt", 80000, 0, "[\"engine_bay\"]", "Inspect annually after 60k"),
            ("Tire Rotation", 5000, 6, "[\"wheels_off\"]", ""),
            ("Wiper Blades", 0, 12, "[\"exterior\"]", ""),
        ];

        for (name, miles, months, categories, notes) in &platform_items {
            let miles_clause = if *miles > 0 { format!("{miles}") } else { "NULL".to_string() };
            let months_clause = if *months > 0 { format!("{months}") } else { "NULL".to_string() };
            let notes_clause = if notes.is_empty() { "NULL".to_string() } else { format!("'{notes}'") };
        ...
        let gti_items = [
            ("Manual Transmission Fluid", 60000, "[\"undercar\"]", "Not in factory schedule but widely recommended"),
            ("Carbon Cleaning (walnut blast)", 80000, "[\"engine_bay\"]", "Direct injection buildup"),
            ("Timing Chain Tensioner Inspection", 80000, "[\"engine_bay\"]", "Known EA888 weak point"),
            ("Water Pump Inspection", 80000, "[\"engine_bay\"]", "Known failure around 80-100k"),
        ];
        ...
        let alltrack_items = [
            ("DSG Fluid & Filter", 40000, "[\"undercar\"]", "Critical for DSG longevity"),
            ("Haldex Fluid & Filter", 40000, "[\"undercar\"]", "Critical for AWD system"),
            ("Carbon Cleaning (walnut blast)", 80000, "[\"engine_bay\"]", "Direct injection buildup"),
            ("Timing Chain Tensioner Inspection", 80000, "[\"engine_bay\"]", "Known EA888 weak point, especially at high mileage"),
            ("Water Pump Inspection", 80000, "[\"engine_bay\"]", "Known failure around 80-100k"),
        ];
