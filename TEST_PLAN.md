...
| 6c | Delete fails (backend error) | The confirm row stays open with the error shown inline (`role="alert"`); the record remains; **Cancel** collapses the row and clears the error. Same behavior on all three delete surfaces (shared ConfirmDelete) |
