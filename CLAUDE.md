...
- `GLOVEBOX_PUBLIC_URL` (default `http://localhost:3003`) — public base URL of the web app, used to build the user-facing deep links MCP hands back (e.g. `record_service`'s browser attach link). `just dev` sets it to the Vite origin `http://localhost:5373`; deployments set their real origin
