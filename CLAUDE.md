...
- `GLOVEBOX_INBOX_DIR` (default `data/inbox`) — the MCP `attach_document` inbox: LLM clients save real files here with their own file tools and pass an inbox-relative `source_path`, so file bytes never travel through model context (point it at the AI client's local files dir)
